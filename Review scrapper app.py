...
def search_apps_by_keywords():
    all_apps = []
    app_names = search(keyword, country='us', lang='en')
    all_apps.extend(app_names)
    return all_apps

# Write app titles and IDs to file
def save_app_list(apps):
    with open("Nasearchmes.txt", "w") as f:
        for app_info in apps:
            f.write(f"{app_info['title']} : {app_info['appId']}\n")
# Get reviews of apps
def fetch_reviews (app_ids,app_titles,user_choice):
    all_reviews = {}
    if user_choice =='keywords': 
        ...
        result, _ = reviews(keyword, country='us', lang='en', sort=Sort.NEWEST, count=100)
...
# Get details of apps
def fetch_app_details (app_ids,user_choice):
    app_details = []
    if user_choice =='keywords':
        ...
        result = app(keyword, country='us', lang='en')
        app_details.append(result)
    return app_details
...
if __name__ == "__main__":
    while True:
        user_choice=input('Keywords or app id : ').lower()
        if user_choice =='keywords':
            keyword=input('Give a keyword : ')
            break
        elif user_choice =='app id':
            keyword = input('give app id : ')
            break
        else:
    ...
    while True:
        ...
        if action == 'search':
            ...
            all_reviews = fetch_reviews(app_ids, app_titles,user_choice)
            save_reviews(app_titles, all_reviews)
            print('Done')
        elif action == 'details':
            app_details = fetch_app_details(app_ids,user_choice)
